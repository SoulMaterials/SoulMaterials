...
// Built-in account titles. These are not crate drops; every new account gets DROOL56.
roleMap.set('member', {id:'member', name:'M-E-M-B-E-R', rarity:'ACCESS', chance:1, baseChance:1, glow:'#52e6ff', value:0, effect:'SSML MEMBER ACCESS'});
roleMap.set('administrative', {id:'administrative', name:'A-D-M-I-N-I-S-T-R-A-T-I-V-E', rarity:'ACCESS', chance:1, baseChance:1, glow:'#ff536e', value:0, effect:'SSML ADMINISTRATIVE ACCESS'});
roleMap.set('drool56', {id:'drool56', name:'D-R-O-O-L-5-6', rarity:'SECRET', chance:1, baseChance:1, glow:'#ffffff', value:0, effect:'Permanent SSML account title'});
...
function ensureAccountShape(a){
  ...
  a.followers ||= [];
  a.friends ||= [];
  a.credits = Number.isFinite(a.credits) ? Math.max(0,Math.floor(a.credits)) : 999000;
  a.access = a.access === "administrative" ? "administrative" : "member";
  a.titleId ||= a.access === "administrative" ? "administrative" : "drool56";
  a.banned = !!a.banned;
...
async function completeProfile(access){
  ...
  if(serverMode){
    try{
      ...
      if(err.status===409){alert("That username already exists. Use SIGN IN instead.");return;}
      if(err.status===403){alert("Administrative access requires the correct proof code.");return;}
      console.error(err); alert("The shared account server could not create the account. Try again in a moment."); return;
    }
  }

  const existing=Object.values(state.accounts).find(a=>a.username.toLowerCase()===username.toLowerCase());
  if(existing){alert("That username already exists. Use SIGN IN instead.");return;}
  const id=uid();
  ...
  const inventory={drool56:1}; if(accessType==="administrative") inventory.administrative=1;
  const a=ensureAccountShape({id,username,avatar,banner,bio,passwordHash:await hashPassword("Drool56"),access:accessType,credits:999000,inventory,activity:[],titleId,equippedTitleId:titleId,following:[],followers:[],friends:[],banned:false});
  state.accounts[id]=a; state.activeUserId=id;
  log(`${username} joined SSML as ${role(titleId).name}`,accessType==="administrative"?"#ff536e":"#52e6ff",a);
...
async function searchDirectory(){
  ...
  if(serverMode){
    try{
      ...
      $("#directoryResults").innerHTML=results.length?results.map(a=>{const r=role(a.equippedTitleId||a.titleId)||role("drool56");const admin=account()?.access==="administrative"?`<button class="directory-gift" data-gift-user="${a.id}">GIFT</button><button class="directory-ban" data-ban-user="${a.id}" data-ban-state="${a.banned?"unban":"ban"}">${a.banned?"UNBAN":"BAN"}</button>`:"";return `<div class="directory-user"><button class="directory-user-main" data-profile-user="${a.id}"><img src="${a.avatar}" alt=""><span><b>${escapeHtml(a.username)}</b><small style="color:${r?.glow||"#fff"}">${escapeHtml(r?.name||"No title")} • ${a.access}</small></span><strong>${money(a.credits)} C</strong></button>${admin}</div>`}).join(""):`<div class="activity"><b>No matching account.</b></div>`;
      return;
    }catch(err){ console.error(err); }
  }
  const results=Object.values(state.accounts).filter(a=>!a.banned && (!q||a.username.toLowerCase().includes(q)));
  $("#directoryResults").innerHTML=results.length?results.map(a=>{const r=role(a.equippedTitleId||a.titleId)||role("drool56");const admin=account()?.access==="administrative"?`<button class="directory-gift" data-gift-user="${a.id}">GIFT</button><button class="directory-ban" data-ban-user="${a.id}" data-ban-state="${a.banned?"unban":"ban"}">${a.banned?"UNBAN":"BAN"}</button>`:"";return `<div class="directory-user"><button class="directory-user-main" data-profile-user="${a.id}"><img src="${a.avatar}" alt=""><span><b>${escapeHtml(a.username)}</b><small style="color:${r?.glow||"#fff"}">${escapeHtml(r?.name||"No title")} • ${a.access}</small></span><strong>${money(a.credits)} C</strong></button>${admin}</div>`}).join(""):`<div class="activity"><b>No matching account.</b></div>`;
}
...
async function searchAdminUsers(){
  ...
  if(serverMode){
    try{
      const data=await apiRequest(`/api/users?q=${encodeURIComponent(q)}&admin=1`);
      data.users.forEach(mergeServerUser);
...
function renderAdminUserList(list){
  $("#adminUserList").innerHTML=list.length?list.map(a=>`<div class="admin-user"><button class="admin-user-main" data-admin-user="${a.id}"><span>${escapeHtml(a.username)}${a.banned?" • BANNED":""}</span><small>${money(a.credits)} C</small></button>${a.id===state.activeUserId?"":`<button class="admin-user-ban" data-ban-user="${a.id}">${a.banned?"UNBAN":"BAN"}</button>`}</div>`).join(""):`<p class="muted">No users found.</p>`;
}
async function banUser(targetId){
  const me=account();
  if(me?.access!=="administrative"||!targetId||targetId===me.id){alert("You cannot ban your own administrative account.");return;}
  const target=state.accounts[targetId];
  const currentlyBanned=!!target?.banned;
  if(!confirm(`${currentlyBanned?"UNBAN":"BAN"} ${target?.username||"this user"}?`))return;
  if(serverMode){
    try{
      const data=await apiRequest('/api/admin/ban',{method:'POST',body:JSON.stringify({adminId:me.id,targetId,banned:!currentlyBanned})});
      mergeServerUser(data.target);
      save();
      if($("#directoryModal")&&!$("#directoryModal").classList.contains("hidden")) await searchDirectory();
      if($("#adminModal")&&!$("#adminModal").classList.contains("hidden")) await searchAdminUsers();
      alert(`${data.target.username} is ${data.target.banned?"now banned":"no longer banned"}.`);
      return;
    }catch(err){console.error(err);alert(err.status===403?"You do not have administrative permission.":"Could not update the ban right now.");return;}
  }
  if(!target)return; target.banned=!currentlyBanned; save();
}
...
async function signInExisting(){
  ...
  if(!username){ alert("Enter your SSML username first."); input?.focus(); return; }

  // Local-first: this makes the button work even if Render is waking up or
  // temporarily unavailable. The server is synced immediately afterward.
  const local=Object.values(state.accounts||{}).find(a=>String(a.username||"").trim().toLowerCase()===username.toLowerCase());
  if(local){
    if(local.banned){ alert("This SSML account is banned."); return; }
    ...
    if(serverMode){
      try{
        const data=await apiRequest('/api/auth/login',{method:'POST',body:JSON.stringify({username})});
        if(data?.user) mergeServerUser(data.user);
        save();
      }catch(err){ console.warn("Server sync during sign-in failed; local account remains active.",err); }
    }
    return;
  }

  if(!serverMode){ alert("That SSML account was not found. Create it first."); return; }

  try{
    const data=await apiRequest('/api/auth/login',{method:'POST',body:JSON.stringify({username})});
    const found=mergeServerUser(data.user);
    if(found.banned){ alert("This SSML account is banned."); return; }
    ...
    if(err.status===404){ alert("That SSML account was not found. Check the username and try again."); return; }
    if(err.status===403){ alert("This SSML account is banned."); return; }
    console.error(err);
    alert("Render is waking up or the shared account server is unavailable. Try the button again in a moment.");
  }
}
...
$("#loginExisting")?.addEventListener("click",(e)=>{e.preventDefault(); signInExisting();});
...
$("#directoryBtn")?.addEventListener("click",openDirectory); $("#directorySearch")?.addEventListener("input",searchDirectory); $("#directoryResults")?.addEventListener("click",e=>{const ban=e.target.closest("[data-ban-user]");if(ban){e.stopPropagation();banUser(ban.dataset.banUser);return;}const gift=e.target.closest("[data-gift-user]");if(gift){e.stopPropagation();$("#giftTargetId").value=gift.dataset.giftUser;const target=state.accounts[gift.dataset.giftUser];$("#giftAmount").value="";showModal("giftModal");return;}const b=e.target.closest("[data-profile-user]");if(b){closeModal("directoryModal");openProfile(b.dataset.profileUser)}});
...
$("#profileAdminBan")?.addEventListener("click",async()=>{const id=$("#profileAdminTarget").value;await banUser(id);if(state.accounts[id]?.banned)closeModal("profileModal");else openProfile(id);});
...
$("#adminUserList")?.addEventListener("click",e=>{const ban=e.target.closest("[data-ban-user]");if(ban){e.stopPropagation();banUser(ban.dataset.banUser);return;}const b=e.target.closest("[data-admin-user]");if(b)openAdminPanel(b.dataset.adminUser)});
